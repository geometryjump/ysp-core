...
$user = $result[0];
$userid = $user["userID"];
$query=$db->prepare("SELECT boosted FROM users WHERE userID = $userid");
$query->execute();
$boost = $query->fetch();
...
//rewards
	...
	//stuff
	$diamond = rand($chest1minDiamonds, $chest1maxDiamonds);
	$orbs = rand($chest1minOrbs, $chest1maxOrbs); //100 = 200
	if($boosted == 1) {
		$diamond = $diamond * 2;
		$orbs = $orbs * 2;
	}
	$chest1stuff = $orbs.",".$diamond.",".rand($chest1minShards, $chest1maxShards).",".rand($chest1minKeys, $chest1maxKeys)."";
	$chest2stuff = $orbs.",".$diamond.",".rand($chest2minShards, $chest2maxShards).",".rand($chest2minKeys, $chest2maxKeys)."";

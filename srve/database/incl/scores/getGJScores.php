...
if($type == "top" OR $type == "creators" OR $type == "relative"){
	if($type == "top"){//top
		...
		$query = "SELECT * FROM users WHERE isBanned = '0' AND extID > 0 AND gameVersion $sign AND stars > 50 ORDER BY stars DESC LIMIT 100";
	}
	if($type == "creators"){//creators
		$query = "SELECT * FROM users WHERE isCreatorBanned = '0' AND extID > 0 AND creatorPoints > 0 ORDER BY creatorPoints DESC LIMIT 100";
	}
	if($type == "relative"){//global

...
foreach($result as &$comment1) {
	if($comment1["commentID"]!=""){
        ...
        $prequery->execute([':userID' => $comment1["userID"]]);
        ...
        $accountid = $gs->getExtID($comment1["userID"]);
        if($prefixdb != "") {
            $prefixdb = "$prefixdb /";
        }
    	...
    	if($roleID != "") {
		    ...
		    $commentstring .= "2~".$actualcomment."~3~".$comment1["userID"]."~4~".$comment1["likes"]."~5~0~7~".$comment1["isSpam"]."~9~".$prefixdb." ".$uploadDate."~6~".$comment1["commentID"]."~10~".$comment1["percent"];

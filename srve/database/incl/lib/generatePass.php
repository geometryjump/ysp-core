	...
	public function isValidUsrname($userName, $pass) {
		...
		if($query6->fetchColumn() < 0){
			...
			if(password_verify($pass, $result["password"])){
				...
				$query = $db->prepare("UPDATE accounts SET ip=:ip WHERE userName=:userName");
				...
				if ($hashed_pass == $result['password']) {
					...
					//updating hash
					$query = $db->prepare("UPDATE accounts SET password=:password,ip=:ip WHERE userName=:userName");

...
foreach($result as $user){
	if($user["userID"] == 8891){
		echo "Not banned (demons) ".htmlspecialchars($user["userName"],ENT_QUOTES)." - ".$user["userID"]."<br>";
	}else{
		//$query = $db->prepare("UPDATE users SET isBanned = '1' WHERE userID = :id");
		//$query->execute([':id' => $user["userID"]]);
		echo "Banned (demons) ".htmlspecialchars($user["userName"],ENT_QUOTES)." - ".$user["userID"]."<br>";
	}
}
//banips

...
if(!empty($_POST["userName"]) AND !empty($_POST["password"]) AND !empty($_POST["packName"]) AND !empty($_POST["levels"]) AND !empty($_POST["stars"]) AND !empty($_POST["coins"]) AND !empty($_POST["color"])){
	...
	if ($pass == 1) {
		...
		if($gs->checkPermission($accountID, "isCronFunction") == false){
			echo "This account doesn't have the permissions to access this tool (isCron). <a href='packCreate.php'>Try again</a>";
		}else{
